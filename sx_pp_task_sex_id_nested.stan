...
model {
  ...
  // likelihood
  for (i in 1:N) {
    sx[i] ~ student_t(nu, b_stage[stage[i]] + b_task[task[i], stage[i]] +b_id[id[i]] +
                      b_sex[sex[i]], sig);
  }
}
...
generated quantities {
  ...
  for (i in 1:N) {
    mu[i] = b_stage[stage[i]] + b_task[task[i], stage[i]] + b_sex[sex[i]] + b_id[id[i]] ;
  }
